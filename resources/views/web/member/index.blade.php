<x-app-layout>
    <div class="min-h-[85vh] mx-auto py-4 px-4 max-w-7xl">
        <div class="flex space-x-1">
            ...
            <a href="{{route('member')}}?tab=following" onclick="display('following')" class="border rounded-full {{$currentTab == 'following' ? 'border-[--primary] bg-[--primary] text-white font-semibold' : 'border-[--secondary] hover:bg-[--secondary] hover:text-white font-semibold' }}">
        ...
        @if($currentTab == "all")
            <div class="flex mt-4 flex-wrap md:justify-start justify-center" id="all">
                @if($items->count() > 0)
                    ...
                    @endforeach 
                @else
                    <p class="text-[--secondary] text-md px-4">No recipe yet.</p>
                @endif
            </div>
