        ...
        <div class="flex mt-4 flex-wrap  md:justify-start justify-center px-1">
            @if($items->count() > 0)
                @foreach($items as $item) 
                    <x-card-recipe class="flex flex-col bg-cover rounded-sm md:w-[33%] w-full md:h-[240px] min-h-[300px] items-center shadow-md bg-blend-overlay md:pt-2 pt-4 bg-center m-[1px]" style="background-image: url('{{ Storage::url($item->thumbnail)}}');" :item="$item"></x-card-recipe>
                @endforeach 
            @else
