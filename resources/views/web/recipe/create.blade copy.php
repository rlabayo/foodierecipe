            ...
            <form method="POST" action="{{ route('recipe.store') }}" class="w-full max-w-2xl" enctype="multipart/form-data">
                ...
                <div class="my-6 mx-auto md:w-1/3 w-full"> 
                    ...
                    @if(session('statusCode') === 201)
                        @include('components.success-modal', ['status'=> session('statusCode'), 'message' => session('message'), 'routeName' => 'profile.index', 'buttonLabel' => 'Back to Profile'])
                    @elseif(session('errorStatusCode') != "")
                        @include('components.error-modal', ['status'=> session('errorStatusCode'), 'message' => session('message')])
                    @endif
                </div>
            </form>
        </div>
