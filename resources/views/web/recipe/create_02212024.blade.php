<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto pt-4 pb-4 px-6 bg-white shadow-lg sm:rounded-lg my-10">
        <h1 class="text-[--secondary] text-2xl font-semibold text-center mt-10">New Recipe</h1>
        <div class="flex flex-wrap mt-4 md:gap-2 gap-[.1rem] justify-center">
            ...
            @endif
            <form method="POST" action="{{ route('recipe.store') }}" class="w-full max-w-2xl" enctype="multipart/form-data">
                @csrf
                ...
                <div class="flex flex-col items-center gap-1 my-4">
                    ...
                    <div class="flex w-full items-center gap-1 mt-2">
                        <div class="w-1/2">
                            <textarea id="instruction_item" class="h-[10rem] w-full border-[--primary] dark:border-primary dark:bg-[--input-dark-bg-color] dark:text-[--secondary] focus:border-[--primary] dark:focus:border-[--primary] focus:ring-[--primary] dark:focus:ring-[--primary] rounded-md shadow-sm"></textarea>
                        </div>
                        <div class="w-1/2 h-[10rem] flex flex-col justify-center items-center ">
                            <div class="flex items-center h-[9rem] justify-center w-full hover:bg-[#ffc5a82e] hover:bg-blend-darken">
                                <label class="flex flex-col">
                                    <div id="preview_photo" class="flex flex-col items-center justify-center w-auto" style="display:none">
                                    ...
                                    <div id="select_photo" class="flex flex-col items-center justify-center w-auto">
                                        ...
                                        <p class="pt-1 px-2 text-sm tracking-wider text-center text-gray-400 group-hover:text-gray-600">
                    ...
                    <span class="text-sm text-red-600 space-y-1 text-start mr-auto" id="instruction_item_error"></span>
                ...
                <div class="my-6 mx-auto md:w-1/3 w-full"> 
                    <x-primary-button class="w-full">{{ __('Post') }}</x-primary-button>
                    @if(session('status') === 201)
                        @include('web.recipe.components.success', ['status'=> 200, 'message' => 'New recipe was successfully added!', 'routeName' => 'profile.index', 'buttonLabel' => 'Back to Profile'])
                    @elseif(session('status') === 400)
                        @include('web.recipe.components.error', ['status'=> 200, 'message' => "Unfortunately we have an issue while saving your new recipe. Please try again! "])
                    @endif
                </div>
            </form>
        </div>
...
@push('script')
    ...
    <script src="{{ Storage::url('assets/js/instructions/iterate_instructions.js') }}" defer></script>
