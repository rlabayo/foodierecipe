        ...
        if(instruction_items.length > 0){
            for(var j = 0; j < instruction_items.length; j++){
                ...
                if(instruction_items[j].attached_photo != undefined && instruction_items[j].attached_photo != ''){
                    var items =  add_instruction_item(j+1, instruction_items[ j ].instruction_item, instruction_items[j].attached_photo, 'update')
                }else{
                    var items =  add_instruction_item(j+1, instruction_items[ j ].instruction_item,'', 'update')
                }
